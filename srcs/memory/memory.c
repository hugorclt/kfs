...
static void	init_memory_var(multiboot_info_t *bootInfo)
{

	for (size_t i = 1; i < bootInfo->mmap_length; i += sizeof(multiboot_memory_map_t)) {
		multiboot_memory_map_t *map_entry = (multiboot_memory_map_t *)(bootInfo->mmap_addr + i);

		if (map_entry->type == MULTIBOOT_MEMORY_AVAILABLE)
		{
			start_physical_memory = map_entry->addr_low;
			total_page_frame = map_entry->len_low / MAX_PAGE_FRAME;
			return ;
		}		
	}
